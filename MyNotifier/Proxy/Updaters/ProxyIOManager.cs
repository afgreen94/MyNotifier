...
{
    public class ProxyIOManager : IProxyIOManager
    {
        private readonly IFileIOManager fileIOManager;
        private readonly ICallContext<ProxyIOManager> callContext;
        ...
        public ProxyIOManager(IFileIOManager fileIOManager, ICallContext<ProxyIOManager> callContext) { this.fileIOManager = fileIOManager; this.callContext = callContext; }

        public async Task<ICallResult> InitializeAsync(bool forceReinitialize)
        {
            try
            {
        ...
        public async Task<ICallResult<IUpdaterDefinition>> RetrieveUpdaterDefinitionAsync(Guid updaterDefinitionId)
        {
            ...
            try
            {
                var fileIOManagerWrapper = new FileIOManager.FileIOManager.Wrapper(this.fileIOManager);

                var updaterDefinitionFilePath = fileIOManagerWrapper.BuildAppendedPath(this.paths.UpdaterDefinitionsFolder.Path, updaterDefinitionId.ToString());
